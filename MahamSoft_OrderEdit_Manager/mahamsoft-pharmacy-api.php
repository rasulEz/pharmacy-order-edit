...
final class Mahamsoft_Pharmacy_API {
    ...
    public function render_modal_html() {
        ...
        ?>
        <style>
        #mwp-ov{display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:199998;}
        #mwp-modal{
            display:none;position:fixed;top:50%;left:50%;
            transform:translate(-50%,-50%);z-index:199999;
            width:680px;max-width:97%;max-height:86vh;overflow-y:auto;
            background:#fff;border-radius:8px;
            box-shadow:0 8px 32px rgba(0,0,0,.28);direction:rtl;
        }
        .mwp-hd{background:#135e96;color:#fff;padding:13px 18px;border-radius:8px 8px 0 0;
            display:flex;justify-content:space-between;align-items:center;gap:10px;}
        .mwp-hd h2{margin:0;font-size:14px;color:#fff;flex:1;overflow:hidden;
            white-space:nowrap;text-overflow:ellipsis;}
        .mwp-hd button{background:none;border:none;color:#fff;font-size:22px;
            cursor:pointer;line-height:1;flex-shrink:0;}
        #mwp-tabs{display:flex;gap:0;border-bottom:2px solid #135e96;
            padding:0 18px;background:#f8f8f8;flex-wrap:wrap;}
        .mwp-tab{padding:9px 16px;cursor:pointer;font-size:13px;font-weight:600;
            border:1px solid #ddd;border-bottom:none;background:#f0f0f0;
            color:#555;border-radius:4px 4px 0 0;margin-left:4px;margin-top:6px;
            white-space:nowrap;}
        .mwp-tab.active{background:#fff;color:#135e96;border-color:#135e96;
            border-bottom:2px solid #fff;margin-bottom:-2px;}
        .mwp-tab.mwp-tab-remove{color:#a33;}
        .mwp-tab.mwp-tab-remove.active{color:#c00;border-color:#c00;border-bottom-color:#fff;}
        .mwp-tab-pane{display:none;padding:16px 18px;}
        .mwp-tab-pane.active{display:block;}
        #mwp-err{background:#fef0f0;color:#c00;border:1px solid #f5c6cb;
            border-radius:4px;padding:9px 13px;margin-bottom:12px;
            display:none;font-size:13px;white-space:pre-wrap;}
        .mwp-load{text-align:center;padding:22px;color:#666;}
        table.mwp-tbl{width:100%;border-collapse:collapse;font-size:13px;margin-bottom:10px;}
        table.mwp-tbl th{background:#f0f0f0;padding:7px 9px;text-align:right;
            border-bottom:2px solid #ddd;font-weight:600;}
        table.mwp-tbl td{padding:7px 9px;border-bottom:1px solid #eee;vertical-align:middle;}
        .mwp-qin{width:70px;padding:4px 6px;border:1px solid #ccc;
            border-radius:4px;text-align:center;font-size:13px;}
        .mwp-qin:focus{border-color:#135e96;outline:none;}
        .mwp-qerr{color:#c00;font-size:11px;display:none;margin-top:2px;}
        .mwp-badge{background:#eaf3e6;color:#276624;border-radius:10px;
            padding:2px 9px;font-size:12px;font-weight:600;display:inline-block;}
        .mwp-sum-row td{background:#f0f6ff;font-weight:600;border-top:2px solid #135e96;}
        .mwp-diff{background:#fff8e6;border:1px solid #f0c040;border-radius:4px;
            padding:8px 12px;font-size:13px;margin-bottom:10px;}
        .mwp-diff.mwp-diff-remove{background:#fdecec;border-color:#f0a0a0;}
        .mwp-need{color:#135e96;font-weight:600;}
        .mwp-need-bad{color:#c00;font-weight:600;}
        #mwp-foot{padding:12px 18px 16px;border-top:1px solid #eee;
            display:flex;gap:8px;justify-content:flex-end;}
        #mwp-ok{background:#135e96;color:#fff;border:none;padding:8px 20px;
            border-radius:4px;cursor:pointer;font-size:13px;font-weight:600;}
        #mwp-ok:hover{background:#0d4474;}
        #mwp-ok:disabled{opacity:.5;cursor:not-allowed;}
        #mwp-cancel{background:#f0f0f0;color:#333;border:1px solid #ccc;
            padding:8px 16px;border-radius:4px;cursor:pointer;font-size:13px;}
        .mwp-retry-btn{background:#e87722;color:#fff;border:none;
            padding:5px 12px;border-radius:4px;cursor:pointer;
            font-size:12px;margin-top:8px;display:block;}
        /* مودال تست اتصال */
        #mwp-co{display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:299998;}
        #mwp-cm{display:none;position:fixed;top:50%;left:50%;
            transform:translate(-50%,-50%);z-index:299999;
            width:420px;max-width:94%;background:#fff;border-radius:8px;
            box-shadow:0 6px 24px rgba(0,0,0,.22);direction:rtl;}
        </style>
        ...
        <div id="mwp-cm">
            ...
            <div style="padding:18px;"><div id="mwp-cr" style="font-size:14px;line-height:1.8;"></div></div>
    ...
    private function find_selection_for_item( $selections, $item_id, $sku ) {
        ...
        if ( ! empty( $sku ) ) {
            foreach ( $selections as $key => $sel ) {
                if ( isset( $sel['sku'] ) && (string) $sel['sku'] === (string) $sku
